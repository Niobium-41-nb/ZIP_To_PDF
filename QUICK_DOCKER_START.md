...
应用默认使用自签名证书。对于生产环境：
1. 获取正式的SSL证书
2. 替换 `cert.pem` 和 `key.pem` 文件
3. 重启容器

### 3. 限制访问
```yaml
# 在 docker-compose.yml 中添加网络限制
networks:
  default:
    driver: bridge
    ipam:
...
# 删除数据（谨慎操作！）
# rm -rf uploads download outputs logs temp
```
...
1. **使用Web界面**: 上传压缩包或输入JM漫画ID
2. **探索API**: 查看 [README.md](README.md) 中的API文档
3. **自定义配置**: 修改 `config.py` 调整应用行为
4. **监控性能**: 使用 `docker stats` 查看资源使用

---

**祝您使用愉快！** 🚀

如果有任何问题或建议，请查看项目文档或提交Issue。